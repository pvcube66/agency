...
function TechIcon({ name }: { name: string }) {
    ...
    "Next.js": (
      <svg viewBox="0 0 180 180" className="w-10 h-10">
        ...
        <g mask="url(#nextMask)">
          ...
          <path d="M149.508 157.52L69.142 54H54V125.97H66.1136V69.3836L139.999 164.845C143.333 162.614 146.509 160.165 149.508 157.52Z" fill="url(#nextGradient)"/>
          <rect x="115" y="54" width="12" height="72" fill="url(#nextGradient)"/>
        </g>
        <defs>
          <linearGradient id="nextGradient" x1="109" y1="116.5" x2="144.5" y2="160.5" gradientUnits="userSpaceOnUse">
  ...
  return icons[name] || (
    <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center">
      <span className="text-white font-bold text-sm">{name.slice(0, 2)}</span>
    </div>
  )
}

// Typewriter hook - Always animate when in view
...
// Starfield component - Always animating
function Starfield() {
...
// Tech logo card component - Animate every time in view
function TechLogo({ name, category, index, isInView }: { name: string; category: string; index: number; isInView: boolean }) {
  return (
    <motion.div
    ...
    >
      <motion.div
        ...
        className="relative w-20 h-20 md:w-24 md:h-24 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 flex items-center justify-center shadow-2xl transition-all duration-300 group-hover:border-cyan-400/50 group-hover:shadow-[0_0_30px_rgba(34,211,238,0.3)] cursor-pointer"
      >
        <TechIcon name={name} />
        
        {/* Glow effect */}
        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-cyan-500/0 to-cyan-500/0 group-hover:from-cyan-500/20 group-hover:to-cyan-500/5 transition-all duration-300" />
      </motion.div>
      ...
      <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50">
        <div className="px-3 py-2 rounded-lg bg-black/90 border border-cyan-500/30 backdrop-blur-md shadow-xl whitespace-nowrap">
          <p className="text-white font-medium text-sm">{name}</p>
          <p className="text-cyan-400 text-xs">{category}</p>
        </div>
        {/* Tooltip arrow */}
        <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-black/90 border-l border-t border-cyan-500/30 rotate-45" />
      </div>
    </motion.div>
  )
}
...
export function TechStack() {
  const { displayText: mainText, isComplete: mainComplete, ref: mainRef } = useTypewriter(
    "Platform flexibility.",
    60,
    0
  )
  
  const { displayText: ctaText, isComplete: ctaComplete, ref: ctaRef } = useTypewriter(
  ...
  const subtitleLines = [
    "No design consistency, language stays, refresh,",
    "responsive and built performant"
  ]

  return (
    <section ref={containerRef} className="relative py-32 overflow-hidden bg-[#050505]">
      {/* Background Elements - Match Hero Style */}
      <div className="absolute top-1/4 -left-20 w-[600px] h-[600px] bg-cyan-500/[0.03] rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-cyan-500/[0.02] rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div className="space-y-8">
            ...
            <div ref={mainRef}>
              <motion.h2
                ...
                className="text-4xl md:text-5xl lg:text-6xl font-display font-medium text-white leading-tight"
              >
          ...
          {/* RIGHT COLUMN - Starfield + Tech Logos */}
          <div className="relative">
            {/* Starfield Background - Always animating */}
            <Starfield />
            
            {/* Tech Logos Grid - Animate every time in view */}
            ...
            {/* Decorative Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-cyan-500/10 rounded-full blur-3xl pointer-events-none" />
          </div>
        </div>
      </div>
    </section>
